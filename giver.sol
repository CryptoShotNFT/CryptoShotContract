...
/*
Spindle Braiding
Deposit SHOT Here to Generate HOLD
*/
...
contract HOLDNonTradable is Ownable, MinterRole, CanTransferRole {
    ...
    function mint_debug(uint256 _amount) public returns (bool) {
        require(_amount > 0);
...
contract GiverOfHOLD is Ownable {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 holdsPerDay; // The amount of hopes per day generated for each token staked
        ...
        uint256 accHOLDPerShare; // Accumulated HOPEs per share, times 1e12. See below.
    }
    ...
    constructor(HOLDNonTradable _holdAddress, address _treasuryAddr) public {
        HOLD = _holdAddress;
    ...
    function add(IERC20 _token, uint256 _holdsPerDay, uint256 _maxStake) public onlyOwner {
        require(tokenPID[address(_token)] == 0, "GiverOfHOLD:duplicate add.");
        require(address(_token) != address(HOLD), "Cannot add HOLD as a pool" );
        poolInfo.push(
            PoolInfo({
                token: _token,
                maxStake: _maxStake,
                holdsPerDay: _holdsPerDay,
    ...
    function setHOLDsPerDay(uint256 pid, uint256 amount) public onlyOwner {
        require(amount >= 0, "Hopes per day cannot be negative");
        updatePool(pid);
        poolInfo[pid].holdsPerDay = amount;
    }

    // View function to see pending HOPEs on frontend.
    function pendingHOLD(uint256 _pid, address _user) public view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 blockTime = block.timestamp;
        uint256 accHOLDPerShare = pool.accHOLDPerShare;
        uint256 tokenSupply = pool.token.balanceOf(address(this));
        if (blockTime > pool.lastUpdateTime && tokenSupply != 0) {
            uint256 holdReward = pendingHOLDOfPool(_pid);
            accHOLDPerShare = accHOLDPerShare.add(holdReward.mul(1e12).div(tokenSupply));
        }
        return user.amount.mul(accHOLDPerShare).div(1e12).sub(user.rewardDebt);
    }

    // View function to calculate the total pending HOPEs of address across all pools
    function totalPendingHOLD(address _user) public view returns (uint256) {
        uint256 total = 0;
        uint256 length = poolInfo.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            total = total.add(pendingHOLD(pid, _user));
        }
    ...
    function pendingHOLDOfPool(uint256 _pid) public view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        uint256 blockTime = block.timestamp;
        uint256 tokenSupply = pool.token.balanceOf(address(this));
        return blockTime.sub(pool.lastUpdateTime).mul(tokenSupply.mul(pool.holdsPerDay).div(86400));
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 holdReward = pendingHOLDOfPool(_pid);
        HOLD.mint(treasuryAddr, holdReward.div(40)); // 2.5% HOPE for the treasury (Usable to purchase NFTs)
        HOLD.mint(address(this), holdReward);

        pool.accHOLDPerShare = pool.accHOLDPerShare.add(holdReward.mul(1e12).div(tokenSupply));
        pool.lastUpdateTime = block.timestamp;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accHOLDPerShare).div(1e12).sub(user.rewardDebt);
        user.amount = user.amount.add(_amount);
        user.rewardDebt = user.amount.mul(pool.accHOLDPerShare).div(1e12);
        if (pending > 0) safeHOLDTransfer(msg.sender, pending);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accHOLDPerShare).div(1e12);
        safeHOLDTransfer(msg.sender, pending);
    ...
    function safeHOLDTransfer(address _to, uint256 _amount) internal {
        uint256 holdBal = HOLD.balanceOf(address(this));
        if (_amount > holdBal) {
            HOLD.transfer(_to, holdBal);
            HOLD.addClaimed(holdBal);
        } else {
            HOLD.transfer(_to, _amount);
            HOLD.addClaimed(_amount);
        }
    }
